...
void CMyGL::SetPcsDef()
{
	...
	for(i=0; i<MAX_DEF; i++)
	{
		dwR = GetRValue(m_pReelMap->m_rgbDef[i]);
		dwG = GetGValue(m_pReelMap->m_rgbDef[i]);
		dwB = GetBValue(m_pReelMap->m_rgbDef[i]);
...
void CMyGL::SetPnlNum()
{
	...
	for (k = 0; k < m_nTotPnl; k++)
	{
		...
		{
			CString strData;
			strData.Format(_T("SetPnlNum: m_pPnlNum[%d] = %d"), k, m_pReelMap->m_pPnlNum[k]);
			SaveLog(strData);
		}

		m_pPnlNum[k] = m_pReelMap->m_pPnlNum[k];
	}
}
...
void CMyGL::SetPnlDefNum()
{
	...
	int k;
	for(k=0; k<m_nTotPnl; k++)
		m_pPnlDefNum[k] = m_pReelMap->m_pPnlDefNum[k];
}
