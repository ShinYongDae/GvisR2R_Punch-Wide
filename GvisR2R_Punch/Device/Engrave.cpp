...
void CEngrave::GetSignalMain(SOCKET_DATA SockData)
{
	...
	if (nCmdCode == _SetSig)
	{
		...
		case _SigInx::_Ready:
			...
			//pView->MpeWrite(_T("MB005503"), (long)SockData.nData1);
		...
		case _SigInx::_Run:
			...
			//pView->MpeWrite(_T("MB005501"), (long)SockData.nData1);
			pView->MpeWrite(_T("MB440162"), 0);
			break;
		case _SigInx::_Reset:
			pDoc->BtnStatus.Main.Reset = (SockData.nData1 > 0) ? TRUE : FALSE;
			//pView->MpeWrite(_T("MB005504"), (long)SockData.nData1);
			pView->MpeWrite(_T("MB440162"), 0);
			break;
		case _SigInx::_Stop:
			...
			//pView->MpeWrite(_T("MB005502"), (long)SockData.nData1);
			pView->MpeWrite(_T("MB440162"), (long)SockData.nData1);
			break;
		case _SigInx::_Auto:
			...
			//pView->MpeWrite(_T("MB005505"), (long)SockData.nData1);
			break;
...
void CEngrave::GetSignalTorqueMotor(SOCKET_DATA SockData)
{
	...
	if (nCmdCode == _SetSig)
	{
		...
		case _SigInx::_MkTq:
			...
			pView->MpeWrite(_T("MB440155"), (long)SockData.nData1);
			break;
		case _SigInx::_AoiTq:
			pDoc->BtnStatus.Tq.Aoi = (SockData.nData1 > 0) ? TRUE : FALSE;
			pView->MpeWrite(_T("MB440156"), (long)SockData.nData1);
			break;
		case _SigInx::_EngTq:
			pDoc->BtnStatus.Tq.Eng = (SockData.nData1 > 0) ? TRUE : FALSE;
			pView->MpeWrite(_T("MB440154"), (long)SockData.nData1);
			break;
			// Is
...
void CEngrave::GetSignalInductionMotor(SOCKET_DATA SockData)
{
	...
	if (nCmdCode == _SetSig)
	{
		...
		case _SigInx::_CcwModRe:
			...
			pView->MpeWrite(_T("MB44017D"), (long)SockData.nData1);
			break;
		case _SigInx::_CcwModUn:
			pDoc->BtnStatus.Induct.Uc = (SockData.nData1 > 0) ? TRUE : FALSE;
			pView->MpeWrite(_T("MB44017C"), (long)SockData.nData1);
			break;
			// Is
...
void CEngrave::GetSignalCore150mm(SOCKET_DATA SockData)
{
	...
	if (nCmdCode == _SetSig)
	{
		...
		case _SigInx::_Core150Re:
			...
			pView->MpeWrite(_T("MB44017E"), (long)SockData.nData1);
			break;
		case _SigInx::_Core150Un:
			pDoc->BtnStatus.Core150.Uc = (SockData.nData1 > 0) ? TRUE : FALSE;
			pView->MpeWrite(_T("MB44017F"), (long)SockData.nData1);
			break;
			// Is
...
void CEngrave::GetSignalEtc(SOCKET_DATA SockData)
{
	...
	if (nCmdCode == _SetSig)
	{
		...
		case _SigInx::_EmgAoi:
			...
			//pView->MpeWrite(_T(""), (long)SockData.nData1);
			break;
			// Is
...
void CEngrave::GetOpInfo(SOCKET_DATA SockData)
{
...
#ifdef USE_MPE
				pView->MpeWrite(_T("MB44017D"), (SockData.nData1 > 0) ? 1 : 0);
#endif
...
#ifdef USE_MPE
				pView->MpeWrite(_T("MB44017C"), (SockData.nData1 > 0) ? 1 : 0);
#endif
...
#ifdef USE_MPE
				if (pView)
					pView->MpeWrite(_T("MB44010E"), pDoc->WorkingInfo.LastJob.bUseAoiUpCleanRoler ? 1 : 0);
#endif
			}
			break;
...
#ifdef USE_MPE
				if (pView)
					pView->MpeWrite(_T("MB44010F"), pDoc->WorkingInfo.LastJob.bUseAoiDnCleanRoler ? 1 : 0);
#endif
			}
			break;
...
#ifdef USE_MPE
				if (pView)
					pView->MpeWrite(_T("MB44016F"), pDoc->WorkingInfo.LastJob.bUseAoiDnUltrasonic ? 1 : 0);
#endif
			}
			break;
...
#ifdef USE_MPE
				if (pView)
					pView->MpeWrite(_T("MB44016E"), pDoc->WorkingInfo.LastJob.bUseEngraveUltrasonic ? 1 : 0);
#endif
			}
			break;
	...
	else if (nCmdCode == _SetData)
	{
		...
		case _ItemInx::_EngItsCode:
			...
			{
				m_bGetOpInfo = TRUE;
				pView->m_mgrReelmap->m_sItsCode = pDoc->WorkingInfo.LastJob.sEngItsCode = CharToString(SockData.strData);
...
void CEngrave::SetStripRatio()
{
	...
	if (bDualTest)
	{
		if (!pView->m_mgrReelmap->m_pReelMapDn || !pView->m_mgrReelmap->m_pReelMapAllUp)
			return;
	}
...
void CEngrave::SetLotCut()
{
	...
	SocketData.nData1 = pDoc->WorkingInfo.LastJob.bLotSep ? 1 : 0;	// pView->m_mgrReelmap->m_pReelMap->m_bUseLotSep
	SendCommand(SocketData);
}
...
void CEngrave::SetDefNumUp()
{
	...
	pView->m_mgrReelmap->m_pReelMapUp->GetPcsNum(nGood, nBad); // 상면
	nTot = nGood + nBad;
...
void CEngrave::SetDefNumDn()
{
	...
	if (bDualTest)
	{		
		pView->m_mgrReelmap->m_pReelMapDn->GetPcsNum(nGood, nBad); // 하면
		nTot = nGood + nBad;
...
void CEngrave::SetDefNumTot()
{
	...
	if (bDualTest)
	{		
		pView->m_mgrReelmap->m_pReelMapAllDn->GetPcsNum(nGood, nBad); // 전체
		nTot = nGood + nBad;
