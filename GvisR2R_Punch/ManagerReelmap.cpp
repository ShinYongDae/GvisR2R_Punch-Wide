...
BOOL CManagerReelmap::InitReelmapUp()
{
	...
	if (pView->m_pDlgMenu01)
		pView->m_pDlgMenu01->SwitchReelmapDisp(pDoc->m_mgrProcedure.m_nSelRmap);
...
BOOL CManagerReelmap::GetAoiInfoUp(int nSerial, int *pNewLot, BOOL bFromBuf) // TRUE: CHANGED, FALSE: NO CHANGED 
{
	...
	if (pDoc->WorkingInfo.LastJob.sModelUp != pDoc->Status.PcrShare[0].sModel 
		...
		|| pDoc->m_mgrProcedure.m_bInitAutoLoadMstInfo)
	{
		pDoc->m_bUpdateForNewJob[0] = TRUE;
		...
		{
			if (!pDoc->m_mgrProcedure.m_bBufEmpty[0])
				pDoc->m_mgrProcedure.m_bBufEmptyF[0] = FALSE;

			pDoc->m_mgrProcedure.m_nAoiCamInfoStrPcs[0] = GetAoiUpCamMstInfo();
			if (pDoc->m_mgrProcedure.m_nAoiCamInfoStrPcs[0] > -1)
			{
		...
		if (pDoc->m_mgrProcedure.m_bInitAutoLoadMstInfo)
		{
			pDoc->m_mgrProcedure.m_bInitAutoLoadMstInfo = FALSE;
			return TRUE;
		}
...
BOOL CManagerReelmap::GetAoiInfoDn(int nSerial, int *pNewLot, BOOL bFromBuf) // TRUE: CHANGED, FALSE: NO CHANGED 
{
	...
	if (pDoc->WorkingInfo.LastJob.sModelDn != pDoc->Status.PcrShare[1].sModel || pDoc->WorkingInfo.LastJob.sLayerDn != pDoc->Status.PcrShare[1].sLayer)
	{
		...
		{
			if (!pDoc->m_mgrProcedure.m_bBufEmpty[1])
				pDoc->m_mgrProcedure.m_bBufEmptyF[1] = FALSE;

			pDoc->m_mgrProcedure.m_nAoiCamInfoStrPcs[1] = GetAoiDnCamMstInfo();
			if (pDoc->m_mgrProcedure.m_nAoiCamInfoStrPcs[1] > -1)
			{
...
BOOL CManagerReelmap::LoadPcrFromBuf() // 프로그램 처음 시작시 버퍼에 저장된 파일을 릴맵에 최초로 업데이트 함.
{
	...
	if (pDoc->m_mgrProcedure.ChkBufUp(pDoc->m_mgrProcedure.m_pBufSerial[0], pDoc->m_mgrProcedure.m_nBufTot[0]))
	{
		...
		{
			LoadPCRUp(pDoc->m_mgrProcedure.m_pBufSerial[0][i]);
			if (!bDualTest)
			{
				UpdateReelmap(pDoc->m_mgrProcedure.m_pBufSerial[0][i]);
				bRtn = TRUE;
			}
		}
	}
	...
	if (bDualTest)
	{
		if (pDoc->m_mgrProcedure.ChkBufDn(pDoc->m_mgrProcedure.m_pBufSerial[1], pDoc->m_mgrProcedure.m_nBufTot[1]))
		{
			...
			{
				LoadPCRDn(pDoc->m_mgrProcedure.m_pBufSerial[1][i]);
				UpdateReelmap(pDoc->m_mgrProcedure.m_pBufSerial[1][i]); // After inspect bottom side.
				bRtn = TRUE;
			}
		}
	}
...
//int CManagerReelmap::GetTotDefPcs0Its(int nSerial)
//{
//	if (nSerial <= 0)
//	{
//		pView->ClrDispMsg();
...
//}
//
//int CManagerReelmap::GetTotDefPcsUp0Its(int nSerial)
//{
//	if (nSerial <= 0)
//	{
//		pView->ClrDispMsg();
...
//}
//
//int CManagerReelmap::GetTotDefPcsDn0Its(int nSerial)
//{
//	if (nSerial <= 0)
//	{
//		pView->ClrDispMsg();
...
//int CManagerReelmap::GetTotDefPcs1Its(int nSerial)
//{
//	if (nSerial <= 0)
//	{
//		pView->ClrDispMsg();
...
//}
//
//int CManagerReelmap::GetTotDefPcsUp1Its(int nSerial)
//{
//	if (nSerial <= 0)
//	{
//		pView->ClrDispMsg();
...
//}
//
//int CManagerReelmap::GetTotDefPcsDn1Its(int nSerial)
//{
//	if (nSerial <= 0)
//	{
//		pView->ClrDispMsg();
...
CfPoint CManagerReelmap::GetMkPnt(int nMkPcs)
{
	...
	if (pDoc->m_mgrReelmap.m_Master[0].m_pPcsRgn)
		ptPnt = pDoc->m_mgrReelmap.m_Master[0].m_pPcsRgn->GetMkPnt(nMkPcs); // Cam0의 Mk 포인트.
#else
	ptPnt.x = 1.0;
	ptPnt.y = 1.0;
#endif
...
//CfPoint CManagerReelmap::GetMkPnt0Its(int nSerial, int nMkPcs) // pcr 시리얼, pcr 불량 피스 읽은 순서 인덱스
//{
//	if (nSerial <= 0)
//	{
//		pView->ClrDispMsg();
...
//CfPoint CManagerReelmap::GetMkPnt1Its(int nSerial, int nMkPcs)
//{
//	if (nSerial <= 0)
//	{
//		pView->ClrDispMsg();
...
//						if (m_Master[0].m_pPcsRgn)
//						{
//							ptPnt = m_Master[0].m_pPcsRgn->GetMkPnt1(nDefPcsId); // Cam1의 Mk 포인트.
//						}
//					}
//				}
//			}
...
//int CManagerReelmap::GetMkStripIdx0Its(int nSerial, int nMkPcs) // 0 : Fail , 1~4 : Strip Idx
//{
//	if (nSerial <= 0)
//	{
//		AfxMessageBox(_T("Serial Error.48"));
...
//}
//
//int CManagerReelmap::GetMkStripIdx1Its(int nSerial, int nMkPcs) // 0 : Fail , 1~4 : Strip Idx
//{
//	if (nSerial <= 0)
//	{
//		pView->ClrDispMsg();
...
BOOL CManagerReelmap::IsFixPcsUp(int nSerial)
{
	...
	if (m_pReelMapUp->IsFixPcs(nSerial, pCol, pRow, nTot, bCont))
	{
		...
		pDoc->m_mgrProcedure.m_sFixMsg[0] = sMsg;
...
BOOL CManagerReelmap::IsFixPcsDn(int nSerial)
{
	...
	if (m_pReelMapDn->IsFixPcs(nSerial, pCol, pRow, nTot, bCont))
	{
		...
		// 		Buzzer(TRUE, 0);
		// 		MsgBox(sMsg);
		pDoc->m_mgrProcedure.m_sFixMsg[1] = sMsg;
		return TRUE;
	}
...
//void CManagerReelmap::WriteChangedModel()
//{
//	BOOL bDualTest = pDoc->WorkingInfo.LastJob.bDualTest;
//
...
//BOOL CManagerReelmap::SetItsSerialInfo(int nItsSerial)
//{
//	BOOL bDualTest = pDoc->WorkingInfo.LastJob.bDualTest;
//
//	CString sName = _T("");
...
BOOL CManagerReelmap::ReloadReelmap(int nSerial)
{
	...
	pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_UP = TRUE;
	if (bDualTest)
	{
		pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_DN = TRUE;
		pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLUP = TRUE;
		pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLDN = TRUE;
	}

	if (pDoc->GetTestMode() == MODE_OUTER)
	{
		pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_UP_INNER = TRUE;
		pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ITS = TRUE;
		if (pDoc->WorkingInfo.LastJob.bDualTestInner)
		{
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_DN_INNER = TRUE;
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLUP_INNER = TRUE;
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLDN_INNER = TRUE;
		}
	}
...
void CManagerReelmap::SetInnerPathAtBuf()
{
...
//void CManagerReelmap::UpdateRstInner()
//{
//	if (m_pDlgMenu06)
...
//BOOL CManagerReelmap::ChkShareIdx(int *pBufSerial, int nBufTot, int nShareSerial)
//{
//	if (nBufTot < 1)
//		return TRUE;
...
//}
//
//BOOL CManagerReelmap::ChkVsShare(int &nSerial)
//{
//	int nS0, nS1;
...
//}
//
//BOOL CManagerReelmap::ChkVsShareUp(int &nSerial)
//{
//	CFileFind cFile;
...
//}
//
//BOOL CManagerReelmap::ChkVsShareDn(int &nSerial)
//{
//	CFileFind cFile;
...
//}
//
//BOOL CManagerReelmap::ChkShare(int &nSerial)
//{
//	int nS0, nS1;
...
//}
//
//BOOL CManagerReelmap::ChkShareUp(int &nSerial)
//{
//	CFileFind cFile;
...
//}
//
//BOOL CManagerReelmap::ChkShareDn(int &nSerial)
//{
//	CFileFind cFile;
...
//}
//
//void CManagerReelmap::ChkShare()
//{
//	ChkShareUp();
//	ChkShareDn();
//}
//
//void CManagerReelmap::ChkShareUp()
//{
//	CString str;
//	int nSerial;
...
//}
//
//void CManagerReelmap::ChkShareDn()
//{
//	CString str;
//	int nSerial;
...
//BOOL CManagerReelmap::ChkBufIdx(int* pSerial, int nTot)
//{
//	if (nTot < 2)
//		return TRUE;
//
...
//}
//
//void CManagerReelmap::SwapUp(__int64 *num1, __int64 *num2) 	// 위치 바꾸는 함수
//{
//	__int64 temp;
//
//	temp = *num2;
...
//}
//
//BOOL CManagerReelmap::SortingInUp(CString sPath, int nIndex)
//{
//	struct _stat buf;
//	struct tm *t;
//
...
//}
//
//BOOL CManagerReelmap::SortingOutUp(int* pSerial, int nTot)
//{
//	int i, k;
//
...
//}
//
//BOOL CManagerReelmap::ChkBufUp(int* pSerial, int &nTot)
//{
//	CFileFind cFile;
...
//}
//
//BOOL CManagerReelmap::ChkBufDn(int* pSerial, int &nTot)
//{
//	CFileFind cFile;
...
//}
//
//void CManagerReelmap::SwapDn(__int64 *num1, __int64 *num2) 	// 위치 바꾸는 함수
//{
//	__int64 temp;
//
//	temp = *num2;
...
//}
//
//BOOL CManagerReelmap::SortingInDn(CString sPath, int nIndex)
//{
//	struct _stat buf;
//	struct tm *t;
//
...
//}
//
//BOOL CManagerReelmap::SortingOutDn(int* pSerial, int nTot)
//{
//	int i, k;
//
//	for (k = 0; k < nTot; k++) 			// 버블 정렬 소스 시작
...
UINT CManagerReelmap::ThreadProc0(LPVOID lpContext)	// UpdateYield()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[3].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATAE_YIELD[0])
		{
			pThread->UpdateYield(pDoc->m_mgrProcedure.m_nSerialTHREAD_UPDATAE_YIELD[0]);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATAE_YIELD[0] = FALSE;
			Sleep(0);
		}
		else if(pDoc->m_mgrProcedure.m_bTHREAD_UPDATAE_YIELD[1] && !pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_UP)
		{
			if (pDoc->WorkingInfo.LastJob.bDualTest)
			{
				if (!pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_DN && !pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ALLUP && !pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ALLDN)
				{
					pThread->UpdateYield(pDoc->m_mgrProcedure.m_nSerialTHREAD_UPDATAE_YIELD[1]);
					pDoc->m_mgrProcedure.m_bTHREAD_UPDATAE_YIELD[1] = FALSE;
					Sleep(0);
				}
			}
			else
...
UINT CManagerReelmap::ThreadProc1(LPVOID lpContext)	// UpdateRMapUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[1].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		{
			if (!pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_UP) // Yield Reelmap
			{
				pThread->UpdateRMapUp(); // Write Reelmap
				pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_UP = FALSE;
				Sleep(0);
			}
			else
				Sleep(30);
...
UINT CManagerReelmap::ThreadProc2(LPVOID lpContext)	// UpdateRMapDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[2].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_DN) // Write Reelmap
		{
			//pThread->m_bTHREAD_UPDATE_REELMAP_ALLUP = TRUE;
			//pThread->m_bTHREAD_UPDATE_REELMAP_ALLDN = TRUE;
			if (!pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_DN) // Yield Reelmap
			{
				pThread->UpdateRMapDn(); // Write Reelmap
				pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_DN = FALSE;
				Sleep(0);
			}
			else
				Sleep(30);
...
UINT CManagerReelmap::ThreadProc3(LPVOID lpContext)	// UpdateRMapAllUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[3].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_ALLUP) // Write Reelmap
		{
			if (!pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ALLUP) // Yield Reelmap
			{
				pThread->UpdateRMapAllUp(); // Write Reelmap
				pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_ALLUP = FALSE;
				Sleep(0);
			}
			else
				Sleep(30);
...
UINT CManagerReelmap::ThreadProc4(LPVOID lpContext)	// UpdateRMapAllDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[4].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_ALLDN) // Write Reelmap
		{
			if (!pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ALLDN) // Yield Reelmap
			{
				pThread->UpdateRMapAllDn(); // Write Reelmap
				pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_ALLDN = FALSE;
				Sleep(0);
			}
			else
				Sleep(30);
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[5].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_UP) // Yield Reelmap
		{
			if (!pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_UP) // Write Reelmap
			{
				...
				pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_UP = FALSE;
				if (!pDoc->WorkingInfo.LastJob.bDualTest) 
				{
					if (pDoc->GetTestMode() == MODE_OUTER)
						pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ITS = TRUE;
				}
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[6].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_DN) // Yield Reelmap
		{
			if (!pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_DN) // Write Reelmap
			{
				...
				pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_DN = FALSE;
				pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ALLUP = TRUE;
				pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ALLDN = TRUE;
				Sleep(0);
			}
			else
				Sleep(30);
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[7].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ALLUP) // Yield Reelmap
		{
			if (!pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_ALLUP) // Write Reelmap
			{
				...
				pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ALLUP = FALSE;
				if (pDoc->GetTestMode() == MODE_OUTER)
					pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ITS = TRUE;
				Sleep(0);
			}
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[8].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ALLDN) // Yield Reelmap
		{
			if (!pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_ALLDN) // Write Reelmap
			{
				...
				pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ALLDN = FALSE;
				Sleep(0);
			}
			else
				Sleep(30);
...
UINT CManagerReelmap::ThreadProc9(LPVOID lpContext)	// ReloadReelmapUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[9].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_UP)
		{
			pThread->ReloadReelmapUp();
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_UP = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc10(LPVOID lpContext)	// ReloadReelmapDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[10].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_DN)
		{
			pThread->ReloadReelmapDn();
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_DN = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc11(LPVOID lpContext)	// ReloadReelmapAllUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[11].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLUP)
		{
			pThread->ReloadReelmapAllUp();
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLUP = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc12(LPVOID lpContext)	// ReloadReelmapAllDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[12].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLDN)
		{
			pThread->ReloadReelmapAllDn();
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLDN = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc13(LPVOID lpContext) // WriteReelmapIts()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[13].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_ITS) // Write Reelmap
		{
			pThread->WriteReelmapIts();
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_ITS = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc14(LPVOID lpContext)	// UpdateReelmapYieldIts()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[14].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ITS) // Yield Reelmap
		{
			...
			pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_ITS = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc15(LPVOID lpContext)	// ReloadReelmapUpInner()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[15].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_UP_INNER)
		{
			pThread->ReloadReelmapUpInner();
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_UP_INNER = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc16(LPVOID lpContext)	// ReloadReelmapDnInner()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[16].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_DN_INNER)
		{
			pThread->ReloadReelmapDnInner();
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_DN_INNER = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc17(LPVOID lpContext)	// ReloadReelmapAllUpInner()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[17].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLUP_INNER)
		{
			pThread->ReloadReelmapAllUpInner();
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLUP_INNER = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc18(LPVOID lpContext)	// ReloadReelmapAllDnInner()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[18].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLDN_INNER)
		{
			pThread->ReloadReelmapAllDnInner();
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ALLDN_INNER = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc19(LPVOID lpContext)	// ReloadReelmapIts()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[19].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ITS)
		{
			pThread->ReloadReelmapIts();
			pDoc->m_mgrProcedure.m_bTHREAD_RELOAD_RST_ITS = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc20(LPVOID lpContext)	// UpdateRMapInnerUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[20].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_INNER_UP) // Write Reelmap
		{
			...
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_INNER_UP = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc21(LPVOID lpContext)	// UpdateRMapInnerDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[21].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_INNER_DN) // Write Reelmap
		{
			//pThread->m_bTHREAD_UPDATE_REELMAP_ALLUP = TRUE;
			//pThread->m_bTHREAD_UPDATE_REELMAP_ALLDN = TRUE;
			pThread->UpdateRMapInnerDn();
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_INNER_DN = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc22(LPVOID lpContext)	// UpdateRMapInnerAllUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[22].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_INNER_ALLUP) // Write Reelmap
		{
			pThread->UpdateRMapInnerAllUp();
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_INNER_ALLUP = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc23(LPVOID lpContext)	// UpdateRMapInnerAllDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[23].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_INNER_ALLDN) // Write Reelmap
		{
			pThread->UpdateRMapInnerAllDn();
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_REELMAP_INNER_ALLDN = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc24(LPVOID lpContext)	// UpdateYieldUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[24].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_UP)
		{
			pThread->UpdateYieldUp(pDoc->m_mgrProcedure.m_nSnTHREAD_UPDATAE_YIELD);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_UP = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc25(LPVOID lpContext)	// UpdateYieldDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[25].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_DN)
		{
			pThread->UpdateYieldDn(pDoc->m_mgrProcedure.m_nSnTHREAD_UPDATAE_YIELD);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_DN = FALSE;
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ALLUP = TRUE;
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ALLDN = TRUE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc26(LPVOID lpContext)	// UpdateYieldAllUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[26].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ALLUP)
		{
			pThread->UpdateYieldAllUp(pDoc->m_mgrProcedure.m_nSnTHREAD_UPDATAE_YIELD);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ALLUP = FALSE;

			if (pDoc->GetTestMode() == MODE_OUTER)
			{
				pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ITS = TRUE;
...
UINT CManagerReelmap::ThreadProc27(LPVOID lpContext)	// UpdateYieldAllDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[27].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ALLDN)
		{
			pThread->UpdateYieldAllDn(pDoc->m_mgrProcedure.m_nSnTHREAD_UPDATAE_YIELD);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ALLDN = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc28(LPVOID lpContext)	// UpdateYieldInnerUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[28].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_UP)
		{
			pThread->UpdateYieldInnerUp(pDoc->m_mgrProcedure.m_nSnTHREAD_UPDATAE_YIELD);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_UP = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc29(LPVOID lpContext)	// UpdateYieldInnerDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[29].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_DN)
		{
			pThread->UpdateYieldInnerDn(pDoc->m_mgrProcedure.m_nSnTHREAD_UPDATAE_YIELD);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_DN = FALSE;
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_ALLUP = TRUE;
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_ALLDN = TRUE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc30(LPVOID lpContext)	// UpdateYieldInnerAllUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[30].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_ALLUP)
		{
			pThread->UpdateYieldInnerAllUp(pDoc->m_mgrProcedure.m_nSnTHREAD_UPDATAE_YIELD);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_ALLUP = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc31(LPVOID lpContext)	// UpdateYieldInnerAllDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[31].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_ALLDN)
		{
			pThread->UpdateYieldInnerAllDn(pDoc->m_mgrProcedure.m_nSnTHREAD_UPDATAE_YIELD);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_INNER_ALLDN = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
UINT CManagerReelmap::ThreadProc32(LPVOID lpContext)	// UpdateYieldIts()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[32].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ITS)
		{
			pThread->UpdateYieldIts(pDoc->m_mgrProcedure.m_nSnTHREAD_UPDATAE_YIELD);
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATE_YIELD_ITS = FALSE;
			Sleep(0);
		}
		else
			Sleep(30);
...
void CManagerReelmap::UpdateRMapUp()
{
	if (m_pReelMapUp)
	{
		m_pReelMapUp->Write(pDoc->m_mgrProcedure.m_nSerialRmapUpdate); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
...
void CManagerReelmap::UpdateRMapDn()
{
	if (m_pReelMapDn)
	{
		m_pReelMapDn->Write(pDoc->m_mgrProcedure.m_nSerialRmapUpdate); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
...
void CManagerReelmap::UpdateRMapAllUp()
{
	if (m_pReelMapAllUp)
	{
		m_pReelMapAllUp->Write(pDoc->m_mgrProcedure.m_nSerialRmapUpdate); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
...
void CManagerReelmap::UpdateRMapAllDn()
{
	if (m_pReelMapAllDn)
	{
		m_pReelMapAllDn->Write(pDoc->m_mgrProcedure.m_nSerialRmapUpdate); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
...
{
	//return pDoc->WriteReelmapIts(pDoc->m_mgrProcedure.m_nSerialRmapUpdate);
	BOOL bRtn = m_pReelMapIts->WriteIts(pDoc->m_mgrProcedure.m_nSerialRmapUpdate);
...
void CManagerReelmap::UpdateRMapInnerUp()
{
	...
	{
		//if(m_pReelMapInnerUp)
		//	m_pReelMapInnerUp->MakeItsFile(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, RMAP_INNER_UP);
		if (m_pReelMapUp)
			m_pReelMapUp->MakeItsFile(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, RMAP_INNER_UP);
	}

	if (pDoc->GetTestMode() == MODE_OUTER)
	{
		if (m_pReelMapInnerUp)
			m_pReelMapInnerUp->Write(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
																	  //m_pReelMapInnerUp->Write(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, 0, m_sPathRmapInnerUpdate[0]); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn

																	  //if (m_pReelMapInnerUp)
																	  //	m_pReelMapInnerUp->MakeItsFile(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, RMAP_INNER_UP);
		if (m_pReelMapInnerUp)
			m_pReelMapInnerUp->MakeItsFile(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, RMAP_UP);
	}
}
...
void CManagerReelmap::UpdateRMapInnerDn()
{
	...
	{
		//if (m_pReelMapInnerDn)
		//	m_pReelMapInnerDn->MakeItsFile(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, RMAP_INNER_DN);
		if (m_pReelMapDn)
			m_pReelMapDn->MakeItsFile(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, RMAP_INNER_DN);
	}

	if (pDoc->GetTestMode() == MODE_OUTER)
	{
		if (m_pReelMapInnerDn)
			m_pReelMapInnerDn->Write(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
																	  //m_pReelMapInnerDn->Write(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, 1, m_sPathRmapInnerUpdate[1]); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn

																	  //if (m_pReelMapInnerDn)
																	  //	m_pReelMapInnerDn->MakeItsFile(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, RMAP_INNER_DN);
		if (m_pReelMapInnerDn)
			m_pReelMapInnerDn->MakeItsFile(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, RMAP_DN);
	}
}
...
void CManagerReelmap::UpdateRMapInnerAllUp()
{
	...
	{
		if (m_pReelMapInnerAllUp)
			m_pReelMapInnerAllUp->Write(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
																		 //m_pReelMapInnerAllUp->Write(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, 2, m_sPathRmapInnerUpdate[2]); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
	}
}
...
void CManagerReelmap::UpdateRMapInnerAllDn()
{
	...
	{
		if (m_pReelMapInnerAllDn)
			m_pReelMapInnerAllDn->Write(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
																		 //m_pReelMapInnerAllDn->Write(pDoc->m_mgrProcedure.m_nSerialRmapInnerUpdate, 3, m_sPathRmapInnerUpdate[3]); // [0]:AOI-Up , [1]:AOI-Dn , [2]:AOI-AllUp , [3]:AOI-AllDn
	}
}
...
void CManagerReelmap::UpdateYieldOnThread(int nSerial)
{
	...
	{
		pDoc->m_mgrProcedure.m_nSerialTHREAD_UPDATAE_YIELD[0] = nSerial;
		pDoc->m_mgrProcedure.m_bTHREAD_UPDATAE_YIELD[0] = TRUE; // Cam[0]
		Sleep(30);
	}
	else
	{
		...
		{
			pDoc->m_mgrProcedure.m_nSerialTHREAD_UPDATAE_YIELD[1] = pDoc->m_mgrProcedure.m_nBufUpSerial[1];
			pDoc->m_mgrProcedure.m_bTHREAD_UPDATAE_YIELD[1] = TRUE; // Cam[1]
			Sleep(30);
		}
...
//BOOL CManagerReelmap::IsBuffer(int nNum)
//{
//	BOOL bDualTest = pDoc->WorkingInfo.LastJob.bDualTest;
...
//}
//
//BOOL CManagerReelmap::IsBufferUp()
//{
//	if (m_nBufTot[0] > 0)
//		return TRUE;
//	return FALSE;
//}
//
//BOOL CManagerReelmap::IsBufferDn()
//{
//	if (m_nBufTot[1] > 0)
//		return TRUE;
//	return FALSE;
//}
//
//int CManagerReelmap::GetBuffer(int *pPrevSerial)
//{
...
//}
//
//int CManagerReelmap::GetBufferUp(int *pPrevSerial)
//{
//	if (IsBufferUp())
...
//}
//
//int CManagerReelmap::GetBufferDn(int *pPrevSerial)
//{
//	if (IsBufferDn())
...
//}
//
//BOOL CManagerReelmap::IsBuffer0()
//{
...
//}
//
//BOOL CManagerReelmap::IsBufferUp0()
//{
//	if (m_nBufTot[0] > 0)
//		return TRUE;
//	return FALSE;
//}
//
//BOOL CManagerReelmap::IsBufferDn0()
//{
//	if (m_nBufTot[1] > 0)
//		return TRUE;
//	return FALSE;
//}
//
//int CManagerReelmap::GetBuffer0(int *pPrevSerial)
//{
...
//}
//
//int CManagerReelmap::GetBufferUp0(int *pPrevSerial)
//{
//	if (IsBufferUp0())
//		return m_pBufSerial[0][0];
//	else if (pPrevSerial)
...
//}
//
//int CManagerReelmap::GetBufferDn0(int *pPrevSerial)
//{
//	if (IsBufferDn0())
//		return m_pBufSerial[1][0];
//	else if (pPrevSerial)
...
//}
//
//BOOL CManagerReelmap::IsBuffer1()
//{
//	if (m_nBufTot[0] > 1 && m_nBufTot[1] > 1)
//		return TRUE;
//	return FALSE;
//}
//
//BOOL CManagerReelmap::IsBufferUp1()
//{
//	if (m_nBufTot[0] > 1)
//		return TRUE;
//	return FALSE;
//}
//
//BOOL CManagerReelmap::IsBufferDn1()
//{
//	if (m_nBufTot[1] > 1)
//		return TRUE;
//	return FALSE;
//}
//
//int CManagerReelmap::GetBuffer1(int *pPrevSerial)
//{
...
//}
//
//int CManagerReelmap::GetBufferUp1(int *pPrevSerial)
//{
//	if (IsBufferUp1())
...
//}
//
//int CManagerReelmap::GetBufferDn1(int *pPrevSerial)
//{
//	if (IsBufferDn1())
...
//}
//
//BOOL CManagerReelmap::IsShareUp()
//{
//	return pDoc->Status.PcrShare[0].bExist;
//}
//
//BOOL CManagerReelmap::IsShareDn()
//{
//	return pDoc->Status.PcrShare[1].bExist;
//}
//
//BOOL CManagerReelmap::IsVsShare()
//{
//	int nSerial;
...
//}
//
//int CManagerReelmap::GetShareUp()
//{
//	return pDoc->Status.PcrShare[0].nSerial;
//}
//
//int CManagerReelmap::GetShareDn()
//{
...
{
	int nS0, nS1;
	if (pDoc->m_mgrProcedure.m_bLoadShare[0])
	{
		nS0 = CopyPcrUp();
		// 		if(pView->m_pDlgFrameHigh)
		// 			pView->m_pDlgFrameHigh->SetAoiLastShot(0, nS0);
	}
	if (pDoc->m_mgrProcedure.m_bLoadShare[1])
	{
		nS1 = CopyPcrDn();
		// 		if(pView->m_pDlgFrameHigh)
	...
	if (pDoc->m_mgrProcedure.m_bLoadShare[0] && pDoc->m_mgrProcedure.m_bLoadShare[1])
	{
		if (nS0 > 0 && nS1 > 0)
			return TRUE;
	}
	else if (pDoc->m_mgrProcedure.m_bLoadShare[0] && !pDoc->m_mgrProcedure.m_bLoadShare[1])
	{
		if (nS0 > 0)
			return TRUE;
	}
...
int CManagerReelmap::LoadPCRAllUpInner(int nSerial, BOOL bFromShare)	// return : 2(Failed), 1(정상), -1(Align Error, 노광불량), -2(Lot End)
{
	...
	int nTotDef[3] = { 0 };										// [0]: 상면, [1]: 하면, [2]: 상/하면 Merge
	nTotDef[0] = pDoc->m_mgrReelmap.m_pPcrInner[0][nIdx]->m_nTotDef;			// 상면 불량 피스 수
	nTotDef[1] = pDoc->m_mgrReelmap.m_pPcrInner[1][nIdx]->m_nTotDef;			// 하면 불량 피스 수
...
int CManagerReelmap::LoadPCRAllDnInner(int nSerial, BOOL bFromShare)	// return : 2(Failed), 1(정상), -1(Align Error, 노광불량), -2(Lot End)
{
	...
	int nTotDef[3];
	nTotDef[0] = pDoc->m_mgrReelmap.m_pPcrInner[0][nIdx]->m_nTotDef;
	nTotDef[1] = pDoc->m_mgrReelmap.m_pPcrInner[1][nIdx]->m_nTotDef;
	//	nTotDef[2] = nTotDef[0] + nTotDef[1];
...
void CManagerReelmap::UpdateYieldOnRmap()
{
	...
	pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_UP = TRUE;		// UpdateReelmapYieldUp(); // Yield Reelmap
	if (bDualTest)
	{
		pDoc->m_mgrProcedure.m_bTHREAD_REELMAP_YIELD_DN = TRUE;	// UpdateReelmapYieldDn(); // Yield Reelmap
	}
	Sleep(100);
}
